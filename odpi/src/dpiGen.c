...
int ob_dpiGen__addRef(void *ptr, dpiHandleTypeNum typeNum, const char *fnName)
{
    ...
    if (ob_dpiGen__startPublicFn(ptr, typeNum, fnName, &error) < 0)
        return ob_dpiGen__endPublicFn(ptr, DPI_FAILURE, &error);
    ob_dpiGen__setRefCount(ptr, &error, 1);
    return ob_dpiGen__endPublicFn(ptr, DPI_SUCCESS, &error);
}


//-----------------------------------------------------------------------------
// ob_dpiGen__allocate() [INTERNAL]
        ...
        dpiError *error)
{
    ...
    if (ob_dpiUtils__allocateMemory(1, typeDef->size, 1, "allocate handle",
            (void**) &value, error) < 0)
        return DPI_FAILURE;
    ...
    if (!env && typeNum != DPI_HTYPE_CONTEXT) {
        if (ob_dpiUtils__allocateMemory(1, sizeof(dpiEnv), 1, "allocate env",
                (void**) &env, error) < 0) {
            ob_dpiUtils__freeMemory(value);
            return DPI_FAILURE;
        }
    }
    value->env = env;
    if (ob_dpiDebugLevel & DPI_DEBUG_LEVEL_REFS)
        ob_dpiDebug__print("ref %p (%s) -> 1 [NEW]\n", value, typeDef->name);
...
//-----------------------------------------------------------------------------
// ob_dpiGen__checkHandle() [INTERNAL]
        ...
        const char *action, dpiError *error)
{
    ...
    if (!ptr || value->typeDef != typeDef ||
            value->checkInt != typeDef->checkInt)
        return ob_dpiError__set(error, action, DPI_ERR_INVALID_HANDLE,
                typeDef->name);
    return DPI_SUCCESS;
}
...
//-----------------------------------------------------------------------------
// ob_dpiGen__endPublicFn() [INTERNAL]
...
int ob_dpiGen__endPublicFn(const void *ptr, int returnValue, dpiError *error)
{
    if (ob_dpiDebugLevel & DPI_DEBUG_LEVEL_FNS)
        ob_dpiDebug__print("fn end %s(%p) -> %d\n", error->buffer->fnName, ptr,
                returnValue);
    if (error->handle)
        ob_dpiHandlePool__release(error->env->errorHandles, &error->handle);
...
//-----------------------------------------------------------------------------
// ob_dpiGen__setRefCount() [INTERNAL]
...
void ob_dpiGen__setRefCount(void *ptr, dpiError *error, int increment)
{
    ...
    if (value->env->threaded)
        ob_dpiMutex__acquire(value->env->mutex);
    value->refCount += increment;
    localRefCount = value->refCount;
    if (localRefCount == 0)
        ob_dpiUtils__clearMemory(&value->checkInt, sizeof(value->checkInt));
    if (value->env->threaded)
        ob_dpiMutex__release(value->env->mutex);

    // reference count debugging
    if (ob_dpiDebugLevel & DPI_DEBUG_LEVEL_REFS)
        ob_dpiDebug__print("ref %p (%s) -> %d\n", ptr, value->typeDef->name,
                localRefCount);
...
//-----------------------------------------------------------------------------
// ob_dpiGen__startPublicFn() [INTERNAL]
        ...
        const char *fnName, dpiError *error)
{
    ...
    if (ob_dpiDebugLevel & DPI_DEBUG_LEVEL_FNS)
        ob_dpiDebug__print("fn start %s(%p)\n", fnName, ptr);
    if (ob_dpiGlobal__initError(fnName, error) < 0)
        return DPI_FAILURE;
    if (ob_dpiGen__checkHandle(ptr, typeNum, "check main handle", error) < 0)
        return DPI_FAILURE;
    error->env = value->env;
    return DPI_SUCCESS;
}

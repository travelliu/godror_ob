...
static int ob_dpiStmt__getQueryInfo(dpiStmt *stmt, uint32_t pos,
        dpiQueryInfo *info, dpiError *error);
static int ob_dpiStmt__getQueryInfoFromParam(dpiStmt *stmt, void *param,
        dpiQueryInfo *info, dpiError *error);
static int ob_dpiStmt__postFetch(dpiStmt *stmt, dpiError *error);
static int ob_dpiStmt__beforeFetch(dpiStmt *stmt, dpiError *error);
static int ob_dpiStmt__reExecute(dpiStmt *stmt, uint32_t numIters,
        uint32_t mode, dpiError *error);


//-----------------------------------------------------------------------------
// ob_dpiStmt__allocate() [INTERNAL]
//   Create a new statement object and return it. In case of error NULL is
// returned.
//-----------------------------------------------------------------------------
int ob_dpiStmt__allocate(dpiConn *conn, int scrollable, dpiStmt **stmt,
        dpiError *error)
{
    dpiStmt *tempStmt;

    *stmt = NULL;
    if (ob_dpiGen__allocate(DPI_HTYPE_STMT, conn->env, (void**) &tempStmt,
            error) < 0)
        return DPI_FAILURE;
        ...
        const char *name, uint32_t nameLength, dpiError *error)
{
    ...
    if (pos == 0 && nameLength == 0)
        return ob_dpiError__set(error, "bind zero length name",
    ...
    if (var->type->oracleTypeNum == DPI_ORACLE_TYPE_STMT) {
        for (i = 0; i < var->buffer.maxArraySize; i++) {
            if (var->buffer.externalData[i].value.asStmt == stmt) {
                return ob_dpiError__set(error, "bind to self",
                        DPI_ERR_NOT_SUPPORTED);
            }
        }
    }
    ...
    if (found) {
        ...
        // otherwise, release previously bound variable, if applicable
            ...
            ob_dpiGen__setRefCount(entry->var, error, -1);
            entry->var = NULL;
        }
    ...
    // if not found, add to the list of bind variables
        ...
        if (stmt->numBindVars == stmt->allocatedBindVars) {
            if (ob_dpiUtils__allocateMemory(stmt->allocatedBindVars + 8,
            ...
            if (stmt->bindVars) {
                ...
                ob_dpiUtils__freeMemory(stmt->bindVars);
            }
        ...
        entry->pos = pos;
        if (name) {
            if (ob_dpiUtils__allocateMemory(1, nameLength, 0,
    ...
    if (var->isDynamic && (stmt->statementType == DPI_STMT_TYPE_BEGIN ||
        ...
        if (ob_dpiVar__convertToLob(var, error) < 0)
            return DPI_FAILURE;
    }

    // perform actual bind
    ob_dpiGen__setRefCount(var, error, 1);
    entry->var = var;
    dynamicBind = stmt->isReturning || var->isDynamic;
    if (pos > 0) {
        if (stmt->env->versionInfo->versionNum < 12)
            status = ob_dpiOci__bindByPos(stmt, &bindHandle, pos, dynamicBind,
                    var, error);
        else status = ob_dpiOci__bindByPos2(stmt, &bindHandle, pos, dynamicBind,
                var, error);
    } else {
        if (stmt->env->versionInfo->versionNum < 12)
            status = ob_dpiOci__bindByName(stmt, &bindHandle, name,
                    (int32_t) nameLength, dynamicBind, var, error);
        else status = ob_dpiOci__bindByName2(stmt, &bindHandle, name,
                (int32_t) nameLength, dynamicBind, var, error);
    }
    ...
    if (status < 0) {
        if (error->buffer->code == 1036) {
            if (stmt->statementType == DPI_STMT_TYPE_CREATE ||
                ...
                ob_dpiError__set(error, error->buffer->action,
    ...
    if (var->type->charsetForm != DPI_SQLCS_IMPLICIT) {
        if (ob_dpiOci__attrSet(bindHandle, DPI_OCI_HTYPE_BIND,
    ...
    if (var->buffer.objectIndicator &&
            ob_dpiOci__bindObject(var, bindHandle, error) < 0)
        return DPI_FAILURE;

    // setup dynamic bind, if applicable
    if (dynamicBind && ob_dpiOci__bindDynamic(var, bindHandle, error) < 0)
        return DPI_FAILURE;
...
//-----------------------------------------------------------------------------
// ob_dpiStmt__check() [INTERNAL]
//   Determine if the statement is open and available for use.
//-----------------------------------------------------------------------------
static int ob_dpiStmt__check(dpiStmt *stmt, const char *fnName, dpiError *error)
{
    if (ob_dpiGen__startPublicFn(stmt, DPI_HTYPE_STMT, fnName, error) < 0)
        return DPI_FAILURE;
    if (!stmt->handle || (stmt->parentStmt && !stmt->parentStmt->handle))
        return ob_dpiError__set(error, "check closed", DPI_ERR_STMT_CLOSED);
    if (ob_dpiConn__checkConnected(stmt->conn, error) < 0)
        return DPI_FAILURE;
    if (stmt->statementType == 0 && ob_dpiStmt__init(stmt, error) < 0)
        return DPI_FAILURE;
    return DPI_SUCCESS;
}
...
//-----------------------------------------------------------------------------
// ob_dpiStmt__clearBatchErrors() [INTERNAL]
//   Clear the batch errors associated with the statement.
//-----------------------------------------------------------------------------
static void ob_dpiStmt__clearBatchErrors(dpiStmt *stmt)
{
    if (stmt->batchErrors) {
        ob_dpiUtils__freeMemory(stmt->batchErrors);
...
//-----------------------------------------------------------------------------
// ob_dpiStmt__clearBindVars() [INTERNAL]
//   Clear the bind variables associated with the statement.
//-----------------------------------------------------------------------------
static void ob_dpiStmt__clearBindVars(dpiStmt *stmt, dpiError *error)
{
    uint32_t i;
    ...
    if (stmt->bindVars) {
        for (i = 0; i < stmt->numBindVars; i++) {
            if (stmt->bindVars[i].var)
                ob_dpiGen__setRefCount(stmt->bindVars[i].var, error, -1);
            if (stmt->bindVars[i].name)
                ob_dpiUtils__freeMemory( (void*) stmt->bindVars[i].name);
        }
        ob_dpiUtils__freeMemory(stmt->bindVars);
        stmt->bindVars = NULL;
    }
...
//-----------------------------------------------------------------------------
// ob_dpiStmt__clearQueryVars() [INTERNAL]
//   Clear the query variables associated with the statement.
//-----------------------------------------------------------------------------
static void ob_dpiStmt__clearQueryVars(dpiStmt *stmt, dpiError *error)
{
    ...
    if (stmt->queryVars) {
        for (i = 0; i < stmt->numQueryVars; i++) {
            if (stmt->queryVars[i]) {
                ob_dpiGen__setRefCount(stmt->queryVars[i], error, -1);
                stmt->queryVars[i] = NULL;
            }
            typeInfo = &stmt->queryInfo[i].typeInfo;
            if (typeInfo->objectType) {
                ob_dpiGen__setRefCount(typeInfo->objectType, error, -1);
                typeInfo->objectType = NULL;
            }
            if (typeInfo->annotations) {
                ob_dpiUtils__freeMemory(typeInfo->annotations);
                typeInfo->annotations = NULL;
            }
        }
        ob_dpiUtils__freeMemory(stmt->queryVars);
        stmt->queryVars = NULL;
    }
    if (stmt->queryInfo) {
        ob_dpiUtils__freeMemory(stmt->queryInfo);
        stmt->queryInfo = NULL;
    }
    stmt->numQueryVars = 0;
}
...
// is called from ob_dpiStmt_close() where errors are expected to be propagated
// and from ob_dpiStmt__free() where errors are ignored.
//-----------------------------------------------------------------------------
int ob_dpiStmt__close(dpiStmt *stmt, const char *tag, uint32_t tagLength,
        int propagateErrors, dpiError *error)
{
    ...
    if (stmt->env->threaded)
        ob_dpiMutex__acquire(stmt->env->mutex);
    closing = stmt->closing;
    stmt->closing = 1;
    if (stmt->env->threaded)
        ob_dpiMutex__release(stmt->env->mutex);
    ...
    ob_dpiStmt__clearBatchErrors(stmt);
    ob_dpiStmt__clearBindVars(stmt, error);
    ob_dpiStmt__clearQueryVars(stmt, error);
    if (stmt->lastRowid)
        ob_dpiGen__setRefCount(stmt->lastRowid, error, -1);
    if (stmt->handle) {
        if (stmt->parentStmt) {
            ob_dpiGen__setRefCount(stmt->parentStmt, error, -1);
            stmt->parentStmt = NULL;
        } else if (!stmt->conn->deadSession && stmt->conn->handle) {
            if (stmt->isOwned)
                ob_dpiOci__handleFree(stmt->handle, DPI_OCI_HTYPE_STMT);
            else status = ob_dpiOci__stmtRelease(stmt, tag, tagLength,
...
//-----------------------------------------------------------------------------
// ob_dpiStmt__createBindVar() [INTERNAL]
//   Create a bind variable given a value to bind.
//-----------------------------------------------------------------------------
static int ob_dpiStmt__createBindVar(dpiStmt *stmt,
        ...
        uint32_t pos, const char *name, uint32_t nameLength, dpiError *error)
{
    ...
    switch (nativeTypeNum) {
        ...
        case DPI_NATIVE_TYPE_BOOLEAN:
            ...
            break;
        default:
            return ob_dpiError__set(error, "create bind var",
                    DPI_ERR_UNHANDLED_CONVERSION, 0, nativeTypeNum);
    }

    // create the variable and set its value
    if (ob_dpiVar__allocate(stmt->conn, oracleTypeNum, nativeTypeNum, 1, size, 1,
            0, objType, &tempVar, &varData, error) < 0)
        return DPI_FAILURE;

    // copy value from source to target data
    if (ob_dpiVar__copyData(tempVar, 0, data, error) < 0)
        return DPI_FAILURE;

    // bind variable to statement
    status = ob_dpiStmt__bind(stmt, tempVar,  pos, name, nameLength, error);
    ob_dpiGen__setRefCount(tempVar, error, -1);
...
//-----------------------------------------------------------------------------
// ob_dpiStmt__createQueryVars() [INTERNAL]
//   Create space for the number of query variables required to support the
// query.
//-----------------------------------------------------------------------------
static int ob_dpiStmt__createQueryVars(dpiStmt *stmt, dpiError *error)
{
    uint32_t numQueryVars, i;

    // determine number of query variables
    if (ob_dpiOci__attrGet(stmt->handle, DPI_OCI_HTYPE_STMT,
    ...
    if (numQueryVars != stmt->numQueryVars) {
        if (ob_dpiUtils__allocateMemory(numQueryVars, sizeof(dpiVar*), 1,
                "allocate query vars", (void**) &stmt->queryVars, error) < 0)
            return DPI_FAILURE;
        if (ob_dpiUtils__allocateMemory(numQueryVars, sizeof(dpiQueryInfo), 1,
        ...
        for (i = 0; i < numQueryVars; i++) {
            if (ob_dpiStmt__getQueryInfo(stmt, i + 1, &stmt->queryInfo[i],
                    error) < 0) {
                ob_dpiStmt__clearQueryVars(stmt, error);
                return DPI_FAILURE;
            }
...
// assumed to be valid.
//-----------------------------------------------------------------------------
static int ob_dpiStmt__define(dpiStmt *stmt, uint32_t pos, dpiVar *var,
        dpiError *error)
{
    ...
    // perform the define
    if (stmt->env->versionInfo->versionNum < 12) {
        if (ob_dpiOci__defineByPos(stmt, &defineHandle, pos, var, error) < 0)
            return DPI_FAILURE;
    } else {
        if (ob_dpiOci__defineByPos2(stmt, &defineHandle, pos, var, error) < 0)
            return DPI_FAILURE;
    }

    // set the charset form if applicable
    if (var->type->charsetForm != DPI_SQLCS_IMPLICIT) {
        if (ob_dpiOci__attrSet(defineHandle, DPI_OCI_HTYPE_DEFINE,
                (void*) &var->type->charsetForm, 0, DPI_OCI_ATTR_CHARSET_FORM,
                "set charset form", error) < 0)
            return DPI_FAILURE;
    }
    ...
    if (var->buffer.objectIndicator && ob_dpiOci__defineObject(var, defineHandle,
            error) < 0)
        return DPI_FAILURE;

    // register callback for dynamic defines
    if (var->isDynamic && ob_dpiOci__defineDynamic(var, defineHandle, error) < 0)
        return DPI_FAILURE;

    // remove previous variable and retain new one
    if (stmt->queryVars[pos - 1])
        ob_dpiGen__setRefCount(stmt->queryVars[pos - 1], error, -1);
    ob_dpiGen__setRefCount(var, error, 1);
...
//-----------------------------------------------------------------------------
// ob_dpiStmt__execute() [INTERNAL]
//   Internal execution of statement.
//-----------------------------------------------------------------------------
static int ob_dpiStmt__execute(dpiStmt *stmt, uint32_t numIters,
        uint32_t mode, int reExecute, dpiError *error)
{
    ...
    for (i = 0; i < stmt->numBindVars; i++) {
        ...
        if (var->isArray && numIters > 1)
            return ob_dpiError__set(error, "bind array var",
                    DPI_ERR_ARRAY_VAR_NOT_SUPPORTED);
        for (j = 0; j < var->buffer.maxArraySize; j++) {
            data = &var->buffer.externalData[j];
            if (ob_dpiVar__setValue(var, &var->buffer, j, data, error) < 0)
                return DPI_FAILURE;
    ...
    if (stmt->statementType == DPI_STMT_TYPE_SELECT) {
        if (ob_dpiOci__attrSet(stmt->handle, DPI_OCI_HTYPE_STMT,
    ...
    if (mode & DPI_MODE_EXEC_SUSPEND_ON_SUCCESS) {
        if (ob_dpiConn__suspendSessionlessTransaction(stmt->conn,
    ...
    if (ob_dpiOci__stmtExecute(stmt, numIters, mode, error) < 0) {
        ob_dpiOci__attrGet(stmt->handle, DPI_OCI_HTYPE_STMT, &tempOffset, 0,
        ...
        switch (error->buffer->code) {
            ...
            case 1007:
                if (reExecute && stmt->statementType == DPI_STMT_TYPE_SELECT)
                    return ob_dpiStmt__reExecute(stmt, numIters, mode, error);
    ...
    if (mode & DPI_MODE_EXEC_SUSPEND_ON_SUCCESS) {
        if (ob_dpiConn__clearTransaction(stmt->conn, error) < 0)
            return DPI_FAILURE;
    }
    ...
    if (ob_dpiUtils__checkClientVersion(stmt->env->versionInfo, 12, 2,
            NULL) == DPI_SUCCESS) {
        if (ob_dpiOci__attrGet(stmt->handle, DPI_OCI_HTYPE_STMT, &sqlId,
    ...
    if (stmt->statementType == DPI_STMT_TYPE_SELECT) {
        temp = 0;
        if (ob_dpiOci__attrSet(stmt->handle, DPI_OCI_HTYPE_STMT, &temp,
    ...
    if (stmt->isReturning || stmt->statementType == DPI_STMT_TYPE_BEGIN ||
        ...
        for (i = 0; i < stmt->numBindVars; i++) {
            var = stmt->bindVars[i].var;
            for (j = 0; j < var->buffer.maxArraySize; j++) {
                if (ob_dpiVar__getValue(var, &var->buffer, j, 0, error) < 0)
                    return DPI_FAILURE;
            }
            var->error = NULL;
        }
    ...
    if (stmt->statementType == DPI_STMT_TYPE_SELECT) {
        ...
        if (!(mode & DPI_MODE_EXEC_PARSE_ONLY) &&
                ob_dpiStmt__createQueryVars(stmt, error) < 0)
            return DPI_FAILURE;
    }
...
//-----------------------------------------------------------------------------
// ob_dpiStmt__fetch() [INTERNAL]
//   Performs the actual fetch from Oracle.
//-----------------------------------------------------------------------------
static int ob_dpiStmt__fetch(dpiStmt *stmt, dpiError *error)
{
    // perform any pre-fetch activities required
    if (ob_dpiStmt__beforeFetch(stmt, error) < 0)
        return DPI_FAILURE;

    // perform fetch
    if (ob_dpiOci__stmtFetch2(stmt, stmt->fetchArraySize, DPI_MODE_FETCH_NEXT, 0,
            error) < 0)
        return DPI_FAILURE;
    ...
    if (ob_dpiStmt__postFetch(stmt, error) < 0)
        return DPI_FAILURE;
...
//-----------------------------------------------------------------------------
// ob_dpiStmt__free() [INTERNAL]
//   Free the memory associated with the statement.
//-----------------------------------------------------------------------------
void ob_dpiStmt__free(dpiStmt *stmt, dpiError *error)
{
    ob_dpiStmt__close(stmt, NULL, 0, 0, error);
    if (stmt->parentStmt) {
        ob_dpiGen__setRefCount(stmt->parentStmt, error, -1);
        stmt->parentStmt = NULL;
    }
    if (stmt->conn) {
        ob_dpiHandleList__removeHandle(stmt->conn->openStmts, stmt->openSlotNum);
        ob_dpiGen__setRefCount(stmt->conn, error, -1);
        stmt->conn = NULL;
    }
    ob_dpiUtils__freeMemory(stmt);
}


//-----------------------------------------------------------------------------
// ob_dpiStmt__getBatchErrors() [INTERNAL]
//   Get batch errors after statement executed with batch errors enabled.
//-----------------------------------------------------------------------------
static int ob_dpiStmt__getBatchErrors(dpiStmt *stmt, dpiError *error)
{
    ...
    if (ob_dpiUtils__allocateMemory(stmt->numBatchErrors, sizeof(dpiErrorBuffer),
    ...
    if (ob_dpiOci__handleAlloc(stmt->env->handle, &localErrorHandle,
    ...
    if (ob_dpiOci__handleAlloc(stmt->env->handle, &batchErrorHandle,
            DPI_OCI_HTYPE_ERROR, "allocate batch error handle", error) < 0) {
        ob_dpiStmt__clearBatchErrors(stmt);
        ob_dpiOci__handleFree(localErrorHandle, DPI_OCI_HTYPE_ERROR);
        return DPI_FAILURE;
    }
    ...
    for (i = 0; i < stmt->numBatchErrors; i++) {
        ...
        if (ob_dpiOci__paramGet(error->handle, DPI_OCI_HTYPE_ERROR,
                &batchErrorHandle, i, "get batch error", error) < 0) {
            overallStatus = ob_dpiError__set(error, "get batch error",
        ...
        if (ob_dpiOci__attrGet(batchErrorHandle, DPI_OCI_HTYPE_ERROR, &rowOffset,
                0, DPI_OCI_ATTR_DML_ROW_OFFSET, "get row offset",
                &localError) < 0) {
            overallStatus = ob_dpiError__set(error, "get row offset",
        ...
        ob_dpiError__setFromOCI(&localError, DPI_OCI_ERROR, stmt->conn,
                "get batch error");
    ...
    // cleanup
    ob_dpiOci__handleFree(localErrorHandle, DPI_OCI_HTYPE_ERROR);
    ob_dpiOci__handleFree(batchErrorHandle, DPI_OCI_HTYPE_ERROR);
    if (overallStatus < 0)
        ob_dpiStmt__clearBatchErrors(stmt);
...
//-----------------------------------------------------------------------------
// ob_dpiStmt__getRowCount() [INTERNAL]
...
//-----------------------------------------------------------------------------
static int ob_dpiStmt__getRowCount(dpiStmt *stmt, uint64_t *count,
        dpiError *error)
{
    uint32_t rowCount32;
    ...
    if (stmt->statementType == DPI_STMT_TYPE_SELECT)
        ...
        *count = 0;
        ...
        if (ob_dpiOci__attrGet(stmt->handle, DPI_OCI_HTYPE_STMT, &rowCount32, 0,
                DPI_OCI_ATTR_ROW_COUNT, "get row count", error) < 0)
            return DPI_FAILURE;
        *count = rowCount32;
    } else {
        if (ob_dpiOci__attrGet(stmt->handle, DPI_OCI_HTYPE_STMT, count, 0,
...
//-----------------------------------------------------------------------------
// ob_dpiStmt__getQueryInfo() [INTERNAL]
        ...
        dpiQueryInfo *info, dpiError *error)
{
    ...
    if (ob_dpiOci__paramGet(stmt->handle, DPI_OCI_HTYPE_STMT, &param, pos,
            "get parameter", error) < 0)
        return DPI_FAILURE;

    // acquire information from the parameter descriptor
    status = ob_dpiStmt__getQueryInfoFromParam(stmt, param, info, error);
    ob_dpiOci__descriptorFree(param, DPI_OCI_DTYPE_PARAM);
    return status;
}


//-----------------------------------------------------------------------------
// ob_dpiStmt__getQueryInfoFromParam() [INTERNAL]
        ...
        dpiQueryInfo *info, dpiError *error)
{
    ...
    // aquire name of item
    if (ob_dpiOci__attrGet(param, DPI_OCI_HTYPE_DESCRIBE, (void*) &info->name,
            &info->nameLength, DPI_OCI_ATTR_NAME, "get name", error) < 0)
        return DPI_FAILURE;

    // acquire type information
    if (ob_dpiOracleType__populateTypeInfo(stmt->conn, param,
            DPI_OCI_HTYPE_DESCRIBE, &info->typeInfo, error) < 0)
        return DPI_FAILURE;

    // acquire if column is permitted to be null
    if (ob_dpiOci__attrGet(param, DPI_OCI_HTYPE_DESCRIBE, (void*) &ociNullOk, 0,
...
int ob_dpiStmt__init(dpiStmt *stmt, dpiError *error)
{
    ...
    else if (ob_dpiOci__attrGet(stmt->handle, DPI_OCI_HTYPE_STMT,
...
//-----------------------------------------------------------------------------
// ob_dpiStmt__postFetch() [INTERNAL]
...
static int ob_dpiStmt__postFetch(dpiStmt *stmt, dpiError *error)
{
    ...
    for (i = 0; i < stmt->numQueryVars; i++) {
        ...
        for (j = 0; j < stmt->bufferRowCount; j++) {
            if (ob_dpiVar__getValue(var, &var->buffer, j, 1, error) < 0)
                return DPI_FAILURE;
...
//-----------------------------------------------------------------------------
// ob_dpiStmt__beforeFetch() [INTERNAL]
...
static int ob_dpiStmt__beforeFetch(dpiStmt *stmt, dpiError *error)
{
    ...
    if (!stmt->queryInfo && ob_dpiStmt__createQueryVars(stmt, error) < 0)
        return DPI_FAILURE;
    for (i = 0; i < stmt->numQueryVars; i++) {
        var = stmt->queryVars[i];
        if (!var) {
            queryInfo = &stmt->queryInfo[i];
            if (ob_dpiVar__allocate(stmt->conn, queryInfo->typeInfo.oracleTypeNum,
                ...
                return DPI_FAILURE;
            if (ob_dpiStmt__define(stmt, i + 1, var, error) < 0)
                return DPI_FAILURE;
            ob_dpiGen__setRefCount(var, error, -1);
        }
        var->error = error;
        ...
        if (var->requiresPreFetch && ob_dpiVar__extendedPreFetch(var,
                &var->buffer, error) < 0)
            return DPI_FAILURE;
    }
...
//-----------------------------------------------------------------------------
// ob_dpiStmt__prepare() [INTERNAL]
//   Prepare a statement for execution.
//-----------------------------------------------------------------------------
int ob_dpiStmt__prepare(dpiStmt *stmt, const char *sql, uint32_t sqlLength,
        const char *tag, uint32_t tagLength, dpiError *error)
{
    if (sql && ob_dpiDebugLevel & DPI_DEBUG_LEVEL_SQL)
        ob_dpiDebug__print("SQL %.*s\n", sqlLength, sql);
    if (ob_dpiOci__stmtPrepare2(stmt, sql, sqlLength, tag, tagLength, error) < 0)
        return DPI_FAILURE;
    if (ob_dpiHandleList__addHandle(stmt->conn->openStmts, stmt,
            &stmt->openSlotNum, error) < 0) {
        ob_dpiOci__stmtRelease(stmt, NULL, 0, 0, error);
        stmt->handle = NULL;
        return DPI_FAILURE;
    }

    return ob_dpiStmt__init(stmt, error);
}


//-----------------------------------------------------------------------------
// ob_dpiStmt__reExecute() [INTERNAL]
        ...
        uint32_t mode, dpiError *error)
{
    ...
    if (ob_dpiOci__attrGet(stmt->handle, DPI_OCI_HTYPE_STMT, (void*) &sql,
    ...
    status = ob_dpiOci__stmtPrepare2(stmt, sql, sqlLength, NULL, 0, &localError);
    newHandle = stmt->handle;
    stmt->handle = origHandle;
    stmt->deleteFromCache = 1;
    if (ob_dpiOci__stmtRelease(stmt, NULL, 0, 1, &localError) < 0 || status < 0)
        return DPI_FAILURE;
    ...
    for (i = 0; i < stmt->numBindVars; i++) {
        ...
        var = bindVar->var;
        bindVar->var = NULL;
        if (ob_dpiStmt__bind(stmt, var, bindVar->pos, bindVar->name,
    ...
    return ob_dpiStmt__execute(stmt, numIters, mode, 0, error);
}


//-----------------------------------------------------------------------------
// ob_dpiStmt_addRef() [PUBLIC]
//   Add a reference to the statement.
//-----------------------------------------------------------------------------
int ob_dpiStmt_addRef(dpiStmt *stmt)
{
    return ob_dpiGen__addRef(stmt, DPI_HTYPE_STMT, __func__);
}


//-----------------------------------------------------------------------------
// ob_dpiStmt_bindByName() [PUBLIC]
//   Bind the variable by name.
//-----------------------------------------------------------------------------
int ob_dpiStmt_bindByName(dpiStmt *stmt, const char *name, uint32_t nameLength,
        dpiVar *var)
{
    dpiError error;
    int status;

    if (ob_dpiStmt__check(stmt, __func__, &error) < 0)
        return ob_dpiGen__endPublicFn(stmt, DPI_FAILURE, &error);
    DPI_CHECK_PTR_NOT_NULL(stmt, name)
    if (ob_dpiGen__checkHandle(var, DPI_HTYPE_VAR, "bind by name", &error) < 0)
        return ob_dpiGen__endPublicFn(stmt, DPI_FAILURE, &error);
    status = ob_dpiStmt__bind(stmt, var, 0, name, nameLength, &error);
    return ob_dpiGen__endPublicFn(stmt, status, &error);
}


//-----------------------------------------------------------------------------
// ob_dpiStmt_bindByPos() [PUBLIC]
//   Bind the variable by position.
//-----------------------------------------------------------------------------
int ob_dpiStmt_bindByPos(dpiStmt *stmt, uint32_t pos, dpiVar *var)
{
    dpiError error;
    int status;
    ...
    if (ob_dpiStmt__check(stmt, __func__, &error) < 0)
        return ob_dpiGen__endPublicFn(stmt, DPI_FAILURE, &error);
    if (ob_dpiGen__checkHandle(var, DPI_HTYPE_VAR, "bind by pos", &error) < 0)
        return ob_dpiGen__endPublicFn(stmt, DPI_FAILURE, &error);
    status = ob_dpiStmt__bind(stmt, var, pos, NULL, 0, &error);
    return ob_dpiGen__endPublicFn(stmt, status, &error);
}


//-----------------------------------------------------------------------------
// ob_dpiStmt_bindValueByName() [PUBLIC]
//   Create a variable and bind it by name.
//-----------------------------------------------------------------------------
int ob_dpiStmt_bindValueByName(dpiStmt *stmt, const char *name,
        uint32_t nameLength, dpiNativeTypeNum nativeTypeNum, dpiData *data)
{
    ...
    status = ob_dpiStmt__createBindVar(stmt, nativeTypeNum, data, &var, 0, name,
            nameLength, &error);
    return ob_dpiGen__endPublicFn(stmt, status, &error);
}
...
//-----------------------------------------------------------------------------
// ob_dpiStmt_bindValueByPos() [PUBLIC]
//   Create a variable and bind it by position.
//-----------------------------------------------------------------------------
int ob_dpiStmt_bindValueByPos(dpiStmt *stmt, uint32_t pos,
        dpiNativeTypeNum nativeTypeNum, dpiData *data)
{
    ...
    if (ob_dpiStmt__check(stmt, __func__, &error) < 0)
        return ob_dpiGen__endPublicFn(stmt, DPI_FAILURE, &error);
    DPI_CHECK_PTR_NOT_NULL(stmt, data)
    status = ob_dpiStmt__createBindVar(stmt, nativeTypeNum, data, &var, pos, NULL,
            0, &error);
    return ob_dpiGen__endPublicFn(stmt, status, &error);
}
...
//-----------------------------------------------------------------------------
// ob_dpiStmt_close() [PUBLIC]
//   Close the statement so that it is no longer usable and all resources have
// been released.
//-----------------------------------------------------------------------------
int ob_dpiStmt_close(dpiStmt *stmt, const char *tag, uint32_t tagLength)
{
    dpiError error;
    int status;
    ...
    status = ob_dpiStmt__close(stmt, tag, tagLength, 1, &error);
    return ob_dpiGen__endPublicFn(stmt, status, &error);
}


//-----------------------------------------------------------------------------
// ob_dpiStmt_define() [PUBLIC]
//   Define the variable that will accept output from the cursor in the
// specified column.
//-----------------------------------------------------------------------------
int ob_dpiStmt_define(dpiStmt *stmt, uint32_t pos, dpiVar *var)
{
    dpiError error;
    int status;
    ...
    if (pos == 0 || pos > stmt->numQueryVars) {
        ob_dpiError__set(&error, "check query position",
                DPI_ERR_QUERY_POSITION_INVALID, pos);
        return ob_dpiGen__endPublicFn(stmt, DPI_FAILURE, &error);
    }
    if (ob_dpiGen__checkHandle(var, DPI_HTYPE_VAR, "check variable", &error) < 0)
        return ob_dpiGen__endPublicFn(stmt, DPI_FAILURE, &error);

    status = ob_dpiStmt__define(stmt, pos, var, &error);
    return ob_dpiGen__endPublicFn(stmt, status, &error);
}


//-----------------------------------------------------------------------------
// ob_dpiStmt_defineValue() [PUBLIC]
...
// intended for subsequent use by ob_dpiStmt_getQueryValue(), which makes use of
// implicitly created variables.
//-----------------------------------------------------------------------------
int ob_dpiStmt_defineValue(dpiStmt *stmt, uint32_t pos,
        ...
        uint32_t size, int sizeIsBytes, dpiObjectType *objType)
{
    ...
    if (ob_dpiVar__allocate(stmt->conn, oracleTypeNum, nativeTypeNum,
            stmt->fetchArraySize, size, sizeIsBytes, 0, objType, &var, &data,
            &error) < 0)
        return ob_dpiGen__endPublicFn(stmt, DPI_FAILURE, &error);
    if (ob_dpiStmt__define(stmt, pos, var, &error) < 0)
        return ob_dpiGen__endPublicFn(stmt, DPI_FAILURE, &error);
    ob_dpiGen__setRefCount(var, &error, -1);
    return ob_dpiGen__endPublicFn(stmt, DPI_SUCCESS, &error);
}


//-----------------------------------------------------------------------------
// ob_dpiStmt_execute() [PUBLIC]
...
//-----------------------------------------------------------------------------
int ob_dpiStmt_execute(dpiStmt *stmt, dpiExecMode mode, uint32_t *numQueryColumns)
{
    uint32_t numIters;
    dpiError error;
    ...
    if (ob_dpiStmt__execute(stmt, numIters, mode, 1, &error) < 0)
        return ob_dpiGen__endPublicFn(stmt, DPI_FAILURE, &error);
    if (numQueryColumns)
...
// support this.
//-----------------------------------------------------------------------------
int ob_dpiStmt_executeMany(dpiStmt *stmt, dpiExecMode mode, uint32_t numIters)
{
    dpiError error;
    uint32_t i;
    ...
    if (stmt->statementType == DPI_STMT_TYPE_SELECT) {
        ob_dpiError__set(&error, "check statement type", DPI_ERR_NOT_SUPPORTED);
        return ob_dpiGen__endPublicFn(stmt, DPI_FAILURE, &error);
    }
    ...
    if ((mode & DPI_MODE_EXEC_BATCH_ERRORS ||
            ...
            stmt->statementType != DPI_STMT_TYPE_DELETE &&
            stmt->statementType != DPI_STMT_TYPE_MERGE) {
        ob_dpiError__set(&error, "check mode", DPI_ERR_EXEC_MODE_ONLY_FOR_DML);
        return ob_dpiGen__endPublicFn(stmt, DPI_FAILURE, &error);
    }
    ...
    for (i = 0; i < stmt->numBindVars; i++) {
        if (stmt->bindVars[i].var->buffer.maxArraySize < numIters) {
            ob_dpiError__set(&error, "check array size",
    ...
    // perform execution
    ob_dpiStmt__clearBatchErrors(stmt);
    if (ob_dpiStmt__execute(stmt, numIters, mode, 0, &error) < 0)
        return ob_dpiGen__endPublicFn(stmt, DPI_FAILURE, &error);

    // handle batch errors if mode was specified
    if (mode & DPI_MODE_EXEC_BATCH_ERRORS) {
        if (ob_dpiStmt__getBatchErrors(stmt, &error) < 0)
            return ob_dpiGen__endPublicFn(stmt, DPI_FAILURE, &error);
    }
...
//-----------------------------------------------------------------------------
// ob_dpiStmt_fetch() [PUBLIC]
//   Fetch a row from the database.
//-----------------------------------------------------------------------------
int ob_dpiStmt_fetch(dpiStmt *stmt, int *found, uint32_t *bufferRowIndex)
{
    dpiError error;
    ...
    if (stmt->bufferRowIndex >= stmt->bufferRowCount) {
        if (stmt->hasRowsToFetch && ob_dpiStmt__fetch(stmt, &error) < 0)
            return ob_dpiGen__endPublicFn(stmt, DPI_FAILURE, &error);
        if (stmt->bufferRowIndex >= stmt->bufferRowCount) {
            *found = 0;
            return ob_dpiGen__endPublicFn(stmt, DPI_SUCCESS, &error);
...
//-----------------------------------------------------------------------------
int ob_dpiStmt_fetchRows(dpiStmt *stmt, uint32_t maxRows,
...
//-----------------------------------------------------------------------------
// ob_dpiStmt_getBatchErrorCount() [PUBLIC]
//   Return the number of batch errors that took place during the last
// execution of the statement.
//-----------------------------------------------------------------------------
int ob_dpiStmt_getBatchErrorCount(dpiStmt *stmt, uint32_t *count)
{
    dpiError error;
...
//-----------------------------------------------------------------------------
// ob_dpiStmt_getBatchErrors() [PUBLIC]
//   Return the batch errors that took place during the last execution of the
// statement.
//-----------------------------------------------------------------------------
int ob_dpiStmt_getBatchErrors(dpiStmt *stmt, uint32_t numErrors,
        dpiErrorInfo *errors)
{
    ...
    if (numErrors < stmt->numBatchErrors) {
        ob_dpiError__set(&error, "check num errors", DPI_ERR_ARRAY_SIZE_TOO_SMALL,
                numErrors);
        return ob_dpiGen__endPublicFn(stmt, DPI_FAILURE, &error);
    }
    for (i = 0; i < stmt->numBatchErrors; i++) {
        tempError.buffer = &stmt->batchErrors[i];
        ob_dpiError__getInfo(&tempError, &errors[i]);
    }
    return ob_dpiGen__endPublicFn(stmt, DPI_SUCCESS, &error);
}
...
//-----------------------------------------------------------------------------
// ob_dpiStmt_getBindCount() [PUBLIC]
...
//-----------------------------------------------------------------------------
int ob_dpiStmt_getBindCount(dpiStmt *stmt, uint32_t *count)
{
    dpiError error;
    int status;
    ...
    if (ob_dpiStmt__check(stmt, __func__, &error) < 0)
        return ob_dpiGen__endPublicFn(stmt, DPI_FAILURE, &error);
    DPI_CHECK_PTR_NOT_NULL(stmt, count)
    status = ob_dpiOci__attrGet(stmt->handle, DPI_OCI_HTYPE_STMT, (void*) count,
...
//-----------------------------------------------------------------------------
// ob_dpiStmt_getBindNames() [PUBLIC]
//   Return the unique names of the bind variables referenced in the prepared
// SQL.
//-----------------------------------------------------------------------------
int ob_dpiStmt_getBindNames(dpiStmt *stmt, uint32_t *numBindNames,
        const char **bindNames, uint32_t *bindNameLengths)
{
    ...
    while (1) {
        if (ob_dpiOci__stmtGetBindInfo(stmt, 8, startLoc, &numFound,
        ...
        for (i = 0; i < numThisPass; i++) {
            ...
            if (numActualBindNames == *numBindNames) {
                ob_dpiError__set(&error, "check num bind names",
...
//-----------------------------------------------------------------------------
// ob_dpiStmt_getFetchArraySize() [PUBLIC]
//   Get the array size used for fetches.
//-----------------------------------------------------------------------------
int ob_dpiStmt_getFetchArraySize(dpiStmt *stmt, uint32_t *arraySize)
{
    dpiError error;
...
int ob_dpiStmt_getImplicitResult(dpiStmt *stmt, dpiStmt **implicitResult)
{
    ...
    if (ob_dpiUtils__checkClientVersion(stmt->env->versionInfo, 12, 1,
            &error) < 0)
        return ob_dpiGen__endPublicFn(stmt, DPI_FAILURE, &error);
    if (ob_dpiOci__stmtGetNextResult(stmt, &handle, &error) < 0)
        return ob_dpiGen__endPublicFn(stmt, DPI_FAILURE, &error);
    *implicitResult = NULL;
    if (handle) {
        if (ob_dpiStmt__allocate(stmt->conn, 0, &tempStmt, &error) < 0)
            return ob_dpiGen__endPublicFn(stmt, DPI_FAILURE, &error);
        tempStmt->handle = handle;
        ob_dpiGen__setRefCount(stmt, &error, 1);
        tempStmt->parentStmt = stmt;
        if (ob_dpiStmt__createQueryVars(tempStmt, &error) < 0) {
...
//-----------------------------------------------------------------------------
// ob_dpiStmt_getInfo() [PUBLIC]
//   Return information about the statement in the provided structure.
//-----------------------------------------------------------------------------
int ob_dpiStmt_getInfo(dpiStmt *stmt, dpiStmtInfo *info)
{
    dpiError error;
...
//-----------------------------------------------------------------------------
// ob_dpiStmt_getLastRowid() [PUBLIC]
...
//-----------------------------------------------------------------------------
int ob_dpiStmt_getLastRowid(dpiStmt *stmt, dpiRowid **rowid)
{
    uint64_t rowCount;
    uint32_t tempSize;
    dpiError error;
    ...
    *rowid = NULL;
    if (stmt->statementType == DPI_STMT_TYPE_INSERT ||
            ...
            stmt->statementType == DPI_STMT_TYPE_DELETE ||
            stmt->statementType == DPI_STMT_TYPE_MERGE) {
        if (ob_dpiStmt__getRowCount(stmt, &rowCount, &error) < 0)
            return ob_dpiGen__endPublicFn(stmt, DPI_FAILURE, &error);
        if (rowCount > 0) {
            if (stmt->lastRowid) {
                ob_dpiGen__setRefCount(stmt->lastRowid, &error, -1);
            ...
            if (ob_dpiRowid__allocate(stmt->conn, &stmt->lastRowid, &error) < 0)
                return ob_dpiGen__endPublicFn(stmt, DPI_FAILURE, &error);
            if (ob_dpiOci__attrGet(stmt->handle, DPI_OCI_HTYPE_STMT,
...
//-----------------------------------------------------------------------------
int ob_dpiStmt_getOciAttr(dpiStmt *stmt, uint32_t attribute, dpiDataBuffer *value,
        uint32_t *valueLength)
{
    ...
    status = ob_dpiOci__attrGet(stmt->handle, DPI_OCI_HTYPE_STMT, &value->asRaw,
...
//-----------------------------------------------------------------------------
// ob_dpiStmt_getPrefetchRows() [PUBLIC]
//   Returns the number of rows that will be prefetched when a query is
// executed.
//-----------------------------------------------------------------------------
int ob_dpiStmt_getPrefetchRows(dpiStmt *stmt, uint32_t *numRows)
{
    dpiError error;
...
//-----------------------------------------------------------------------------
// ob_dpiStmt_getQueryInfo() [PUBLIC]
//   Get query information for the position in question.
//-----------------------------------------------------------------------------
int ob_dpiStmt_getQueryInfo(dpiStmt *stmt, uint32_t pos, dpiQueryInfo *info)
{
    dpiError error;
    ...
    if (stmt->env->context->dpiMinorVersion > 1) {
        ...
        memcpy(info, &stmt->queryInfo[pos - 1], sizeof(ob_dpiQueryInfo__v51));
    } else {
        memcpy(info, &stmt->queryInfo[pos - 1], sizeof(ob_dpiQueryInfo__v50));
    }
...
//-----------------------------------------------------------------------------
// ob_dpiStmt_getQueryValue() [PUBLIC]
//   Get value from query at specified position.
//-----------------------------------------------------------------------------
int ob_dpiStmt_getQueryValue(dpiStmt *stmt, uint32_t pos,
        dpiNativeTypeNum *nativeTypeNum, dpiData **data)
{
    dpiError error;
    dpiVar *var;
    ...
    DPI_CHECK_PTR_NOT_NULL(stmt, data)
    if (!stmt->queryVars) {
        ob_dpiError__set(&error, "check query vars", DPI_ERR_QUERY_NOT_EXECUTED);
        return ob_dpiGen__endPublicFn(stmt, DPI_FAILURE, &error);
    }
    if (pos == 0 || pos > stmt->numQueryVars) {
        ob_dpiError__set(&error, "check query position",
    ...
    if (!var || stmt->bufferRowIndex == 0 ||
        ...
        ob_dpiError__set(&error, "check fetched row", DPI_ERR_NO_ROW_FETCHED);
        return ob_dpiGen__endPublicFn(stmt, DPI_FAILURE, &error);
    }
...
//-----------------------------------------------------------------------------
// ob_dpiStmt_getRowCount() [PUBLIC]
//   Return the number of rows affected by the last DML executed (for insert,
// update, delete and merge) or the number of rows fetched (for queries). In
// all other cases, 0 is returned.
//-----------------------------------------------------------------------------
int ob_dpiStmt_getRowCount(dpiStmt *stmt, uint64_t *count)
{
    dpiError error;
    int status;
    ...
    if (ob_dpiStmt__check(stmt, __func__, &error) < 0)
        return ob_dpiGen__endPublicFn(stmt, DPI_FAILURE, &error);
    DPI_CHECK_PTR_NOT_NULL(stmt, count)
    status = ob_dpiStmt__getRowCount(stmt, count, &error);
    return ob_dpiGen__endPublicFn(stmt, status, &error);
}


//-----------------------------------------------------------------------------
// ob_dpiStmt_getRowCounts() [PUBLIC]
//   Return the number of rows affected by each of the iterations executed
// using ob_dpiStmt_executeMany().
//-----------------------------------------------------------------------------
int ob_dpiStmt_getRowCounts(dpiStmt *stmt, uint32_t *numRowCounts,
        uint64_t **rowCounts)
{
    dpiError error;
    int status;
    ...
    if (ob_dpiUtils__checkClientVersion(stmt->env->versionInfo, 12, 1,
            &error) < 0)
        return ob_dpiGen__endPublicFn(stmt, DPI_FAILURE, &error);
    status = ob_dpiOci__attrGet(stmt->handle, DPI_OCI_HTYPE_STMT, rowCounts,
...
//-----------------------------------------------------------------------------
// ob_dpiStmt_getSubscrQueryId() [PUBLIC]
//   Return the query id for a query registered using this statement.
//-----------------------------------------------------------------------------
int ob_dpiStmt_getSubscrQueryId(dpiStmt *stmt, uint64_t *queryId)
{
    dpiError error;
    int status;
    ...
    status = ob_dpiOci__attrGet(stmt->handle, DPI_OCI_HTYPE_STMT, queryId, 0,
...
//-----------------------------------------------------------------------------
// ob_dpiStmt_release() [PUBLIC]
//   Release a reference to the statement.
//-----------------------------------------------------------------------------
int ob_dpiStmt_release(dpiStmt *stmt)
{
    return ob_dpiGen__release(stmt, DPI_HTYPE_STMT, __func__);
}


//-----------------------------------------------------------------------------
// ob_dpiStmt_scroll() [PUBLIC]
//   Scroll to the specified location in the cursor.
//-----------------------------------------------------------------------------
int ob_dpiStmt_scroll(dpiStmt *stmt, dpiFetchMode mode, int32_t offset,
        int32_t rowCountOffset)
{
    ...
    switch (mode) {
        ...
        case DPI_MODE_FETCH_RELATIVE:
            ...
            break;
        default:
            ob_dpiError__set(&error, "scroll mode", DPI_ERR_NOT_SUPPORTED);
            return ob_dpiGen__endPublicFn(stmt, DPI_FAILURE, &error);
    }
    ...
    if (desiredRow < 1 && mode != DPI_MODE_FETCH_LAST) {
        ...
        ob_dpiError__set(&error, "check result set bounds",
    ...
    if (ob_dpiOci__stmtFetch2(stmt, numRows, mode, offset, &error) < 0)
        return ob_dpiGen__endPublicFn(stmt, DPI_FAILURE, &error);
    ...
    if (ob_dpiOci__attrGet(stmt->handle, DPI_OCI_HTYPE_STMT, &currentPosition, 0,
...
//-----------------------------------------------------------------------------
// ob_dpiStmt_setFetchArraySize() [PUBLIC]
...
//-----------------------------------------------------------------------------
int ob_dpiStmt_setFetchArraySize(dpiStmt *stmt, uint32_t arraySize)
{
    dpiError error;
    dpiVar *var;
    uint32_t i;
...
//-----------------------------------------------------------------------------
// ob_dpiStmt_setOciAttr() [PUBLIC]
//   Set the OCI attribute directly. This is intended for testing of attributes
// not currently exposed by ODPI-C and should only be used for that purpose.
//-----------------------------------------------------------------------------
int ob_dpiStmt_setOciAttr(dpiStmt *stmt, uint32_t attribute, void *value,
        uint32_t valueLength)
{
    dpiError error;
    int status;
    ...
    status = ob_dpiOci__attrSet(stmt->handle, DPI_OCI_HTYPE_STMT, value,
...
//-----------------------------------------------------------------------------
// ob_dpiStmt_setPrefetchRows() [PUBLIC]
//   Set the number of rows to prefetch when a query is executed.
//-----------------------------------------------------------------------------
int ob_dpiStmt_setPrefetchRows(dpiStmt *stmt, uint32_t numRows)
{
    dpiError error;
...
//-----------------------------------------------------------------------------
// ob_dpiStmt_deleteFromCache() [PUBLIC]
...
//-----------------------------------------------------------------------------
int ob_dpiStmt_deleteFromCache(dpiStmt *stmt)
{
    dpiError error;

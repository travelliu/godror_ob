...
//-----------------------------------------------------------------------------
// ob_dpiSodaDb__checkConnected() [INTERNAL]
//   Check to see that the connection to the database is available for use.
//-----------------------------------------------------------------------------
static int ob_dpiSodaDb__checkConnected(dpiSodaDb *db, const char *fnName,
        dpiError *error)
{
    if (ob_dpiGen__startPublicFn(db, DPI_HTYPE_SODA_DB, fnName, error) < 0)
        return DPI_FAILURE;
...
//-----------------------------------------------------------------------------
// ob_dpiSodaDb__createDocument() [INTERNAL]
//   Create a document and set the supplied values, if applicable.
//-----------------------------------------------------------------------------
static int ob_dpiSodaDb__createDocument(dpiSodaDb *db, const char *key,
        ...
        dpiError *error)
{
    ...
    if (ob_dpiSodaDoc__allocate(db, NULL, &tempDoc, error) < 0)
        return DPI_FAILURE;

    // set key, if applicable
    if (key && keyLength > 0) {
        if (ob_dpiOci__attrSet(tempDoc->handle, DPI_OCI_HTYPE_SODA_DOCUMENT,
                (void*) key, keyLength, DPI_OCI_ATTR_SODA_KEY, "set key",
                error) < 0) {
            ob_dpiSodaDoc__free(tempDoc, error);
            return DPI_FAILURE;
        }
    }
...
//-----------------------------------------------------------------------------
// ob_dpiSodaDb__getCollectionNames() [PUBLIC]
...
//-----------------------------------------------------------------------------
static int ob_dpiSodaDb__getCollectionNames(dpiSodaDb *db, void *cursorHandle,
        uint32_t limit, dpiStringList *names, dpiError *error)
{
    ...
    while (names->numStrings < limit || limit == 0) {
        ...
        if (ob_dpiOci__sodaCollGetNext(db->conn, cursorHandle, &collectionHandle,
                error) < 0)
            return DPI_FAILURE;
        if (!collectionHandle)
            break;
        ...
        if (ob_dpiOci__attrGet(collectionHandle, DPI_OCI_HTYPE_SODA_COLLECTION,
                (void*) &name, &nameLength, DPI_OCI_ATTR_SODA_COLL_NAME,
                "get collection name", error) < 0) {
            ob_dpiOci__handleFree(collectionHandle, DPI_OCI_HTYPE_SODA_COLLECTION);
            return DPI_FAILURE;
        }

        // add element to list
        if (ob_dpiStringList__addElement(names, name, nameLength,
...
//-----------------------------------------------------------------------------
// ob_dpiSodaDb__free() [INTERNAL]
//   Free the memory for a SODA database.
//-----------------------------------------------------------------------------
void ob_dpiSodaDb__free(dpiSodaDb *db, dpiError *error)
{
    if (db->conn) {
        ob_dpiGen__setRefCount(db->conn, error, -1);
        db->conn = NULL;
    }
    ob_dpiUtils__freeMemory(db);
}


//-----------------------------------------------------------------------------
// ob_dpiSodaDb_addRef() [PUBLIC]
//   Add a reference to the SODA database.
//-----------------------------------------------------------------------------
int ob_dpiSodaDb_addRef(dpiSodaDb *db)
{
    return ob_dpiGen__addRef(db, DPI_HTYPE_SODA_DB, __func__);
}


//-----------------------------------------------------------------------------
// ob_dpiSodaDb_createCollection() [PUBLIC]
//   Create a new SODA collection with the given name and metadata.
//-----------------------------------------------------------------------------
int ob_dpiSodaDb_createCollection(dpiSodaDb *db, const char *name,
        ...
        uint32_t flags, dpiSodaColl **coll)
{
    ...
    // validate parameters
    if (ob_dpiSodaDb__checkConnected(db, __func__, &error) < 0)
        return ob_dpiGen__endPublicFn(db, DPI_FAILURE, &error);
    ...
    // create collection
    if (ob_dpiOci__sodaCollCreateWithMetadata(db, name, nameLength, metadata,
            metadataLength, mode, &handle, &error) < 0)
        return ob_dpiGen__endPublicFn(db, DPI_FAILURE, &error);
    if (ob_dpiSodaColl__allocate(db, handle, coll, &error) < 0) {
        ob_dpiOci__handleFree(handle, DPI_OCI_HTYPE_SODA_COLLECTION);
        return ob_dpiGen__endPublicFn(db, DPI_FAILURE, &error);
    }
    return ob_dpiGen__endPublicFn(db, DPI_SUCCESS, &error);
}


//-----------------------------------------------------------------------------
// ob_dpiSodaDb_createDocument() [PUBLIC]
...
// the collection.
//-----------------------------------------------------------------------------
int ob_dpiSodaDb_createDocument(dpiSodaDb *db, const char *key,
        uint32_t keyLength, const char *content, uint32_t contentLength,
        ...
        dpiSodaDoc **doc)
{
    ...
    // create document
    status = ob_dpiSodaDb__createDocument(db, key, keyLength, content,
            contentLength, mediaType, mediaTypeLength, doc, &error);
    return ob_dpiGen__endPublicFn(db, status, &error);
}


//-----------------------------------------------------------------------------
// ob_dpiSodaDb_createJsonDocument() [PUBLIC]
...
//-----------------------------------------------------------------------------
int ob_dpiSodaDb_createJsonDocument(dpiSodaDb *db, const char *key,
        ...
        dpiSodaDoc **doc)
{
    ...
    if (ob_dpiUtils__checkClientVersion(db->env->versionInfo, 23, 4, &error) < 0)
        return ob_dpiGen__endPublicFn(db, DPI_FAILURE, &error);

    // create document
    if (ob_dpiSodaDb__createDocument(db, key, keyLength, NULL, 0, NULL, 0,
            &tempDoc, &error) < 0)
        return ob_dpiGen__endPublicFn(db, DPI_FAILURE, &error);
    ...
    if (content) {
        jsonDesc = 1;
        status = ob_dpiOci__attrSet(tempDoc->handle, DPI_OCI_HTYPE_SODA_DOCUMENT,
                &jsonDesc, 0, DPI_OCI_ATTR_SODA_JSON_DESC,
                "set JSON descriptor flag", &error);
        if (status == DPI_SUCCESS) {
            status = ob_dpiOci__attrGet(tempDoc->handle,
                    DPI_OCI_HTYPE_SODA_DOCUMENT, (void*) &jsonHandle,
                    &tempLength, DPI_OCI_ATTR_SODA_CONTENT,
                    "get JSON descriptor", &error);
        }
        if (status == DPI_SUCCESS)
            status = ob_dpiJson__allocate(db->conn, jsonHandle, &tempDoc->json,
                    &error);
        if (status == DPI_SUCCESS)
            status = ob_dpiJson__setValue(tempDoc->json, content, &error);
        if (status != DPI_SUCCESS) {
            ob_dpiSodaDoc__free(tempDoc, &error);
            return ob_dpiGen__endPublicFn(db, DPI_FAILURE, &error);
        }
...
//-----------------------------------------------------------------------------
// ob_dpiSodaDb_freeCollectionNames() [PUBLIC]
//   Free the names of the collections allocated earlier with a call to
// ob_dpiSodaDb_getCollectionNames(). This method is deprecated and should be
// replaced with a call to ob_dpiContext_freeStringList().
//-----------------------------------------------------------------------------
int ob_dpiSodaDb_freeCollectionNames(dpiSodaDb *db, dpiStringList *names)
{
    dpiError error;
    ...
    ob_dpiStringList__free(names);
    return ob_dpiGen__endPublicFn(db, DPI_SUCCESS, &error);
}


//-----------------------------------------------------------------------------
// ob_dpiSodaDb_getCollections() [PUBLIC]
//   Return a cursor to iterate over the SODA collections in the database.
//-----------------------------------------------------------------------------
int ob_dpiSodaDb_getCollections(dpiSodaDb *db, const char *startName,
        uint32_t startNameLength, uint32_t flags, dpiSodaCollCursor **cursor)
{
    ...
    if (flags & DPI_SODA_FLAGS_ATOMIC_COMMIT)
        mode |= DPI_OCI_SODA_ATOMIC_COMMIT;
    if (ob_dpiOci__sodaCollList(db, startName, startNameLength, &handle, mode,
            &error) < 0)
        return ob_dpiGen__endPublicFn(db, DPI_FAILURE, &error);
    if (ob_dpiSodaCollCursor__allocate(db, handle, cursor, &error) < 0) {
        ob_dpiOci__handleFree(handle, DPI_OCI_HTYPE_SODA_COLL_CURSOR);
        return ob_dpiGen__endPublicFn(db, DPI_FAILURE, &error);
    }
    return ob_dpiGen__endPublicFn(db, DPI_SUCCESS, &error);
}
...
//-----------------------------------------------------------------------------
// ob_dpiSodaDb_getCollectionNames() [PUBLIC]
//   Return the names of all collections in the provided array.
//-----------------------------------------------------------------------------
int ob_dpiSodaDb_getCollectionNames(dpiSodaDb *db, const char *startName,
        ...
        dpiStringList *names)
{
    ...
    status = ob_dpiSodaDb__getCollectionNames(db, handle, limit, names, &error);
    ob_dpiOci__handleFree(handle, DPI_OCI_HTYPE_SODA_COLL_CURSOR);
    if (status < 0)
        ob_dpiStringList__free(names);
    return ob_dpiGen__endPublicFn(db, status, &error);
...
//-----------------------------------------------------------------------------
// ob_dpiSodaDb_openCollection() [PUBLIC]
//   Open an existing SODA collection and return a handle to it.
//-----------------------------------------------------------------------------
int ob_dpiSodaDb_openCollection(dpiSodaDb *db, const char *name,
        uint32_t nameLength, uint32_t flags, dpiSodaColl **coll)
{
    ...
    if (flags & DPI_SODA_FLAGS_ATOMIC_COMMIT)
        mode |= DPI_OCI_SODA_ATOMIC_COMMIT;
    if (ob_dpiOci__sodaCollOpen(db, name, nameLength, mode, &handle,
            &error) < 0)
        return ob_dpiGen__endPublicFn(db, DPI_FAILURE, &error);
    *coll = NULL;
    if (handle) {
...
//-----------------------------------------------------------------------------
// ob_dpiSodaDb_release() [PUBLIC]
//   Release a reference to the SODA database.
//-----------------------------------------------------------------------------
int ob_dpiSodaDb_release(dpiSodaDb *db)
{
    return ob_dpiGen__release(db, DPI_HTYPE_SODA_DB, __func__);
}

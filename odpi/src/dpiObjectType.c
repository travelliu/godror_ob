...
static int ob_dpiObjectType__init(dpiObjectType *objType, void *handle,
        uint32_t handleType, dpiError *error);


//-----------------------------------------------------------------------------
// ob_dpiObjectType__allocate() [INTERNAL]
        ...
        dpiObjectType **objType, dpiError *error)
{
    ...
    *objType = NULL;
    if (ob_dpiGen__allocate(DPI_HTYPE_OBJECT_TYPE, conn->env,
    ...
    if (ob_dpiObjectType__init(tempObjType, handle, handleType, error) < 0) {
        ob_dpiObjectType__free(tempObjType, error);
        return DPI_FAILURE;
    }
...
//-----------------------------------------------------------------------------
// ob_dpiObjectType__check() [INTERNAL]
//   Validate that the connection from which the object type was created is
// still connected and issue an error if it is not.
//-----------------------------------------------------------------------------
static int ob_dpiObjectType__check(dpiObjectType *objType, const char *fnName,
        dpiError *error)
{
    if (ob_dpiGen__startPublicFn(objType, DPI_HTYPE_OBJECT_TYPE, fnName,
            error) < 0)
        return DPI_FAILURE;
    return ob_dpiConn__checkConnected(objType->conn, error);
}


//-----------------------------------------------------------------------------
// ob_dpiObjectType__describe() [INTERNAL]
...
//-----------------------------------------------------------------------------
static int ob_dpiObjectType__describe(dpiObjectType *objType,
        void *describeHandle, dpiError *error)
{
    ...
    // describe the type
    if (ob_dpiOci__describeAny(objType->conn, objType->tdo, 0, DPI_OCI_OTYPE_PTR,
            describeHandle, error) < 0)
        return DPI_FAILURE;

    // get top level parameter descriptor
    if (ob_dpiOci__attrGet(describeHandle, DPI_OCI_HTYPE_DESCRIBE, &param, 0,
            DPI_OCI_ATTR_PARAM, "get top level parameter", error) < 0)
        return DPI_FAILURE;

    // determine type code
    if (ob_dpiOci__attrGet(param, DPI_OCI_DTYPE_PARAM, &typeCode, 0,
            DPI_OCI_ATTR_TYPECODE, "get type code", error) < 0)
        return DPI_FAILURE;
    objType->typeCode = typeCode;

    // determine the schema of the type
    if (ob_dpiUtils__getAttrStringWithDup("get schema", param,
    ...
    if (objType->env->versionInfo->versionNum > 11) {
        if (ob_dpiUtils__getAttrStringWithDup("get package name", param,
    ...
    if (ob_dpiOci__attrGet(param, DPI_OCI_DTYPE_PARAM,
    ...
    if (typeCode == DPI_SQLT_NCO) {
        ...
        if (ob_dpiOci__attrGet(param, DPI_OCI_DTYPE_PARAM, &collectionParam, 0,
                DPI_OCI_ATTR_COLLECTION_ELEMENT, "get collection descriptor",
                error) < 0)
            return DPI_FAILURE;

        // determine type of element
        if (ob_dpiOracleType__populateTypeInfo(objType->conn, collectionParam,
...
//-----------------------------------------------------------------------------
// ob_dpiObjectType__free() [INTERNAL]
//   Free the memory for an object type.
//-----------------------------------------------------------------------------
void ob_dpiObjectType__free(dpiObjectType *objType, dpiError *error)
{
    if (objType->conn) {
        ob_dpiGen__setRefCount(objType->conn, error, -1);
        objType->conn = NULL;
    }
    if (objType->elementTypeInfo.objectType) {
        ob_dpiGen__setRefCount(objType->elementTypeInfo.objectType, error, -1);
        objType->elementTypeInfo.objectType = NULL;
    }
    if (objType->schema) {
        ob_dpiUtils__freeMemory((void*) objType->schema);
        objType->schema = NULL;
    }
    if (objType->name) {
        ob_dpiUtils__freeMemory((void*) objType->name);
        objType->name = NULL;
    }
    if (objType->packageName) {
        ob_dpiUtils__freeMemory((void*) objType->packageName);
        objType->packageName = NULL;
    }
    ob_dpiUtils__freeMemory(objType);
}


//-----------------------------------------------------------------------------
// ob_dpiObjectType__init() [INTERNAL]
        ...
        uint32_t handleType, dpiError *error)
{
    ...
    if (ob_dpiOci__attrGet(handle, handleType, (void*) &tdoReference, 0,
            DPI_OCI_ATTR_REF_TDO, "get TDO reference", error) < 0)
        return DPI_FAILURE;
    if (ob_dpiOci__objectPin(objType->env->handle, tdoReference, &objType->tdo,
            error) < 0)
        return DPI_FAILURE;

    // acquire a describe handle
    if (ob_dpiOci__handleAlloc(objType->env->handle, &describeHandle,
            DPI_OCI_HTYPE_DESCRIBE, "allocate describe handle", error) < 0)
        return DPI_FAILURE;

    // describe the type
    if (ob_dpiObjectType__describe(objType, describeHandle, error) < 0) {
...
//-----------------------------------------------------------------------------
// ob_dpiObjectType__isXmlType() [INTERNAL]
...
//-----------------------------------------------------------------------------
// ob_dpiObjectType_addRef() [PUBLIC]
//   Add a reference to the object type.
//-----------------------------------------------------------------------------
int ob_dpiObjectType_addRef(dpiObjectType *objType)
{
    return ob_dpiGen__addRef(objType, DPI_HTYPE_OBJECT_TYPE, __func__);
}


//-----------------------------------------------------------------------------
// ob_dpiObjectType_createObject() [PUBLIC]
//   Create a new object of the specified type and return it. Return NULL on
// error.
//-----------------------------------------------------------------------------
int ob_dpiObjectType_createObject(dpiObjectType *objType, dpiObject **obj)
{
    dpiError error;
    int status;

    // validate parameters
    if (ob_dpiObjectType__check(objType, __func__, &error) < 0)
        return ob_dpiGen__endPublicFn(objType, DPI_FAILURE, &error);
    DPI_CHECK_PTR_NOT_NULL(objType, obj)
    status = ob_dpiObject__allocate(objType, NULL, NULL, NULL, obj, &error);
    return ob_dpiGen__endPublicFn(objType, status, &error);
}


//-----------------------------------------------------------------------------
// ob_dpiObjectType_getAttributes() [PUBLIC]
//   Get the attributes for the object type in the provided array.
//-----------------------------------------------------------------------------
int ob_dpiObjectType_getAttributes(dpiObjectType *objType, uint16_t numAttributes,
        dpiObjectAttr **attributes)
{
    ...
    if (numAttributes < objType->numAttributes) {
        ob_dpiError__set(&error, "get attributes", DPI_ERR_ARRAY_SIZE_TOO_SMALL,
                numAttributes);
        return ob_dpiGen__endPublicFn(objType, DPI_FAILURE, &error);
    }
    if (numAttributes == 0)
        return ob_dpiGen__endPublicFn(objType, DPI_SUCCESS, &error);
    ...
    if (ob_dpiOci__attrGet(describeHandle, DPI_OCI_HTYPE_DESCRIBE, &topLevelParam,
    ...
    if (ob_dpiOci__attrGet(topLevelParam, DPI_OCI_DTYPE_PARAM,
    ...
    for (i = 0; i < objType->numAttributes; i++) {
        if (ob_dpiOci__paramGet(attrListParam, DPI_OCI_DTYPE_PARAM, &attrParam,
                (uint32_t) i + 1, "get attribute param", &error) < 0) {
            ob_dpiOci__handleFree(describeHandle, DPI_OCI_HTYPE_DESCRIBE);
            return ob_dpiGen__endPublicFn(objType, DPI_FAILURE, &error);
        }
        if (ob_dpiObjectAttr__allocate(objType, attrParam, &attributes[i],
                &error) < 0) {
            ob_dpiOci__handleFree(describeHandle, DPI_OCI_HTYPE_DESCRIBE);
            return ob_dpiGen__endPublicFn(objType, DPI_FAILURE, &error);
        }
...
//-----------------------------------------------------------------------------
// ob_dpiObjectType_getInfo() [PUBLIC]
//   Return information about the object type.
//-----------------------------------------------------------------------------
int ob_dpiObjectType_getInfo(dpiObjectType *objType, dpiObjectTypeInfo *info)
{
    dpiError error;

    if (ob_dpiGen__startPublicFn(objType, DPI_HTYPE_OBJECT_TYPE, __func__,
            &error) < 0)
        return ob_dpiGen__endPublicFn(objType, DPI_FAILURE, &error);
    ...
    if (objType->env->context->dpiMinorVersion > 1) {
        ...
        memcpy(&info->elementTypeInfo, &objType->elementTypeInfo,
                sizeof(ob_dpiDataTypeInfo__v51));
    } else {
        memcpy(&info->elementTypeInfo, &objType->elementTypeInfo,
                sizeof(ob_dpiDataTypeInfo__v50));
    }
...
//-----------------------------------------------------------------------------
// ob_dpiObjectType_release() [PUBLIC]
//   Release a reference to the object type.
//-----------------------------------------------------------------------------
int ob_dpiObjectType_release(dpiObjectType *objType)
{
    return ob_dpiGen__release(objType, DPI_HTYPE_OBJECT_TYPE, __func__);
}
